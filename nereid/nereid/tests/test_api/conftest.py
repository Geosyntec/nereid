...
from nereid.tests.utils import get_payload
...
def named_validation_responses(client):
    ...
    slow_valid = json.dumps(
        nxGraph_to_dict(
            nx.relabel_nodes(nx.gnr_graph(15000, p=0.05, seed=42), lambda x: str(x))
        )
    )
    slow_invalid = json.dumps(
        nxGraph_to_dict(
            nx.relabel_nodes(nx.gnc_graph(15000, seed=42), lambda x: str(x))
        )
    )
...
def named_subgraph_responses(client):
    ...
    slow_graph = nxGraph_to_dict(
        nx.relabel_nodes(nx.gnr_graph(200, p=0.05, seed=42), lambda x: str(x))
    )
