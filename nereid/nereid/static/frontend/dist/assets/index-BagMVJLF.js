...
<div class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
  <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    ...
    <!-- end tab content -->
    `}_render(){let e=this;h.select(`#${this.parent_id}`).append("div").html(e._template()),new We(`.${this.classname}`).init()}}class He extends v{constructor(e){super({store:_,id:e.id,children:e.children}),this.store.events.subscribe("updateScenarioName",this._update_scenario_name.bind(this))}get scenario_name(){return m(this.store,"state.scenario_name")}_update_scenario_name(){let e=this;h.select(`#${e.id}`).select("#scenario-name").text(e.scenario_name)}_template(){return`
...
`,callback:async function(){var e;let n=await re();if(((e=n==null?void 0:n.status)==null?void 0:e.toLowerCase())==="success"){let t=m(n,"data.results")||[],s=m(n,"data.leaf_results")||[];this.store.dispatch("newResults",{results:t.concat(s)})}}}),ht=new L({id:"find_node",title:"Find",icon:`
...
`,callback:function(){this.store.dispatch("changedPanel",{active_panel:"find-node-panel"})}});class gt extends v{constructor(e){super({store:_,id:e.id,children:e.children})}_template(){return`
  ...
  >
    ...
    </div>`}_render(){let e=this;e.element=h.select(`#${e.parent_id}-content`).append("div"),e.element.html(e._template());let t=e.element.select("form");t.attr("method","get").on("submit",function(s){s.preventDefault(),t.selectAll("input").property("disabled",!1);const a=new FormData(s.target),r=z(Object.fromEntries(a.entries()));return e.primary_callback.bind(e)(r),!1})}}const $t=new bt({id:"find-node-drawer",title:"Find Node",children:[new St({id:"find-node-ui"})],exit_callback:()=>{m(_,"state.graph.nodes").map(e=>me(e)),_.dispatch("foundNodeIds",{}),_.dispatch("editorUpdate")}}),jt=n=>{n.color="orange",n.size=40},me=n=>{n.color=void 0,n.size=void 0},Ot=n=>{let e=m(_,"state.graph.nodes"),t=e.filter(a=>a.id.includes(n));e.map(a=>me(a));let s=[];for(let a of t)a&&s.push(a);if(s.length){for(let a of s)jt(a);_.dispatch("foundNodeIds",{found_node_ids:s.map(a=>a.id)})}_.dispatch("editorUpdate")};class Tt extends v{constructor(e){super({store:_})}get selected_node_id(){return m(this,"store.state.selected_node_id")}update(){let e=this.selected_node_id?`Node: ${this.selected_node_id.toString()}`:'<div class="pt-20">🡄  Select a Node in the Editor Map<div>';this.element.html(e)}_render(){let e=this;e.element=h.select(`#${e.parent_id}-content`).append("div").classed("text-lg font-bold pb-2",!0),e.update(),e.store.events.subscribe("changedSelectedNode",()=>e.update())}}class Lt extends v{constructor(e){super({store:_});let t=this;t.store.events.subscribe("changedSelectedNode",()=>t.update())}get nodes(){return m(this,"store.state.graph.nodes")}get selected_node_id(){return m(this,"store.state.selected_node_id")}toggleNodeType(){let e=this.element.select("input:checked").property("value");this.store.dispatch("nodeEditorType",{node_editor_type:e||"none"})}_template(){return`
<div id="type-form" class="flex items-center justify-center">
  ...
  <!-- Component End  -->
      ...
      `);s.append("div").attr("id",a);let u=t.landSurfaceTableOptions();t.table=await Nt(`#${a}`,u),i.on("click",()=>t.table.undo()),c.on("click",()=>t.table.redo()),s.append("div").classed("flex flex-row justify-end py-4",!0).append("button").classed("btn btn-blue",!0).on("click",()=>{this.setCurrentNodeDataToTableData.bind(this)()}).text("Apply")}async update(){let e=this;if(e.node_editor_type!=="land_surface"){e.element.classed("hidden",!0);return}e.element.classed("hidden",!1);let t=`${e.parent_id}-content-editor`;await e.buildLandSurfaceTable({id:t})}_render(){let e=this;e.element=h.select(`#${e.parent_id}-content`).append("div").classed("mt-4",!0).classed("hidden",!0),e.update(),e.store.events.subscribe("nodeEditorType",()=>e.update()),e.store.events.subscribe("changedSelectedNode",()=>e.update()),e.store.events.subscribe("changedPanel",()=>e.update()),e.store.events.subscribe("newGraph",()=>e.update())}}class Dt extends v{constructor(e){super({store:_})}get node_editor_type(){return m(this,"store.state.node_editor_type")}get selected_node(){let e=m(this,"store.state.selected_node_id");return m(this,"store.state.graph.nodes").find(s=>s.id===e)}get config(){let{schema:e,facility_types:t,facility_type_map:s,facility_alias_map:a,facility_label_map:r}=m(this,"store.state");return{schema:e,facility_types:t,facility_type_map:s,facility_alias_map:a,facility_label_map:r}}update(){let e=this;e.element.html("").classed("flex flex-col w-full",!0),e.element.classed("hidden",e.node_editor_type!=="treatment_facility");let t=e.element.append("select").classed("px-2 my-4 h-8 border rounded-md",!0).attr("id","facility-picker").on("change",function(){var f,g,S,y,k,O;let r=e.element.select("#facility-picker").property("value");if(!r.length)return!1;let i=e.config.facility_type_map[r],c=w(m(e,"selected_node.data")||{});Array.isArray(c)&&(c={});let u={node_id:e.selected_node.id,facility_type:r,ref_data_key:((g=(f=e.selected_node)==null?void 0:f.data)==null?void 0:g.ref_data_key)||((S=e.selected_node)==null?void 0:S.ref_data_key),design_storm_depth_inches:((k=(y=e.selected_node)==null?void 0:y.data)==null?void 0:k.design_storm_depth_inches)||((O=e.selected_node)==null?void 0:O.design_storm_depth_inches)};u=Object.assign(c,u);const p=b=>{e.selected_node.node_type=e.node_editor_type,e.selected_node.data=Object.assign({facility_type:e.element.select("#facility-picker").property("value")},w(b)),e.selected_node.id=b.node_id,e.store.dispatch("newGraph")};let l=m(e,"store.state.nereid_state"),d=m(e,"store.state.nereid_region"),o=m(e,`store.state.treatment_facility_fields.${l}.${d}`)||m(e,"store.state.treatment_facility_fields.state.region");console.debug("tmnt facility fields:",o),At("#facility-form",e.config.schema[i],u,(o==null?void 0:o.disabled)||[],(o==null?void 0:o.ignored)||[],p.bind(e))});e.element.append("div").attr("id","facility-form");let s=w(e.config.facility_types||[]);s.unshift(""),t.selectAll("option").data(s).enter().append("option").attr("value",r=>r&&e.config.facility_label_map[r]).text(r=>r);let a=m(e,"selected_node.data.facility_type");a&&(e.element.select("#facility-picker").property("value",a),t.on("change")())}_render(){let e=this;e.element=h.select(`#${e.parent_id}-content`).append("div"),e.id=`${e.parent_id}-content-treatment-form`,e.element.attr("id",e.id),e.update(),e.store.events.subscribe("stateChange",()=>this.update())}}function At(n,e,t,s,a,r){let i=[...e.required],c=Object.keys(e.properties).filter(o=>!i.includes(o));i.push(...c);let u=[];for(const o of i.filter(f=>!a.includes(f))){let f=e.properties[o];f.name=o,u.push(f)}let p=h.select(n);p.html("");let l=p.append("form");l.attr("id",n.replace("#","")+"-form").attr("name",n.replace("#","")+"-form").attr("method","get").on("submit",function(o){o.preventDefault(),l.selectAll("input").property("disabled",!1);const f=new FormData(o.target),g=z(Object.fromEntries(f.entries()));return r&&r(g),!1}),l.selectAll("div").data(u).enter().append("div").classed("form-group flex flex-row w-full gap-4 text-sm my-4 h-8 items-center",!0).classed("required",o=>e.required.includes(o.name)).each(function(o,f){let g=e.required.includes(o.name),S=(o.description||o.name)+(g?" (required)":" (optional)"),y=h.select(this);y.append("label").classed("control-label py-2 flex w-1/2 ",!0).attr("title",S).text(o.title);let k;function O(b){if((b==null?void 0:b.anyOf)==null)return b;for(let E of["boolean","number","string"])for(let j of b.anyOf)if(j.type===E)switch(j.type){case"boolean":return b.type="boolean",b.default=(j==null?void 0:j.default)||!1,b;case"number":return b.type="number",b;case"string":return b.type="string",b}return b}switch(o=O(o),o.type){case"string":k=y.append("input").attr("name",o.name).attr("id",o.name).attr("title",S).property("required",e.required.includes(o.name||"")).property("disabled",s.includes(o.name)).classed("border-2 rounded-md px-2 py-1",!0),k.attr("type","text").attr("pattern",(o==null?void 0:o.pattern)||".*").attr("value",[o==null?void 0:o.example,o==null?void 0:o.default,""].find(b=>b!=null));break;case"number":k=y.append("input").attr("name",o.name).attr("id",o.name).attr("title",S).property("required",e.required.includes(o.name||"")).property("disabled",s.includes(o.name)).classed("border-2 rounded-md px-2 py-1",!0),k.attr("type","number").attr("step","0.001").attr("value",[o==null?void 0:o.example,o==null?void 0:o.default,""].find(b=>b!=null));break;case"boolean":k=y.append("div").classed("flex flex-row",!0);for(let b of[!0,!1]){let E=o.name+b.toString(),j=k.append("div").classed("flex flex-row items-center",!0);j.append("input").classed("form-check-input p-2",!0).classed("p-2",!0).attr("type","radio").attr("name",o.name).attr("id",E).attr("title",S).property("checked",o.default==b).property("disabled",s.includes(o.name)).attr("value",b),j.append("label").classed("p-2",!0).attr("for",E).attr("title",S).text(b.toString())}break}});for(let[o,f]of Object.entries(t))if(["true","false"].includes(f))for(let g of["true","false"])h.select("#"+o+g).property("checked",f==g);else h.select("#"+o).property("value",f);let d=l.append("div").classed("flex justify-end",!0);d.append("div"),d.append("button").classed("btn btn-blue",!0).attr("type","submit").text("Apply")}const Rt=new Tt,zt=new Lt,Ft=new Ct,Pt=new Dt,It=new vt({id:"node-editor-drawer",title:"Edit Nodes",children:[Rt,zt,Ft,Pt]});class U extends v{constructor(e){super({store:_});let t=this;t.id=e.id,t.title=e.title,t.data=[{}],t.data_callback=e.data_callback||function(s){console.debug("loaded data: ",s)},t.primary_callback=e.primary_callback||function(){console.debug(`clicked primary button load files UI ${t.id}`)},t.primary_button_label=e.primary_button_label||"Apply",t.secondary_callback=e.secondary_callback||null,t.secondary_button_label=e.secondary_button_label||"Update"}_template_primary_button(){return this.primary_callback==null?"":`<button
    class="
      rounded-r-lg
      text-xs
      h-10
  ...
  >
    ...
    </div>`}loadFileAsJson(e,t){let s=e.target.files[0];if(!s.type.match("csv|excel|json")){alert("csv or json please");return}let a=this,r=new FileReader;r.onload=function(i){return function(c){let u;i.type.match("csv|excel")?u=h.csvParse(c.target.result):i.type.match("json")&&(u=JSON.parse(c.target.result)),u=Array.isArray(u)?u.map(p=>z(p)):u,a.data=u,e.target.value=""}}(s),r.readAsText(s)}_render(){let e=this;e.element=h.select(`#${e.parent_id}-content`).append("div"),e.element.html(e._template()),e.primary_button=e.element.select(`#${this.id}-primary-button`),e.primary_button.on("click",()=>{e.data_callback.bind(e)(e.data),e.primary_callback.bind(e)(),e.element.select(`#${this.id} label`)}),e.secondary_button=e.element.select(`#${this.id}-secondary-button`),e.secondary_button.on("click",()=>{e.data_callback.bind(e)(e.data),e.secondary_callback.bind(e)(),e.element.select(`#${this.id} label`)}),e.input=e.element.select(`#${this.id} input`),e.input.on("change",t=>{e.loadFileAsJson.bind(e)(t);let s=e.element.select(`#${this.id} label`);s.text(t.target.files[0].name),s.classed("font-bold",!0);let a=s.node();a.className=a.className.replace(/\btext-gray-.+?\b/g,""),a.classList.add("text-gray-700")})}}const Vt=new U({id:"graph-file-loader",title:"Load Graph (.csv)",primary_callback:V,secondary_callback:ue,data_callback:function(e){let t=[...new Set(e.map(p=>p.source).concat(e.map(p=>p.target)))],s=w(this.store.state.graph.nodes.filter(p=>t.includes(p.id))||[{}]),a=s.map(p=>p.id),r=[],i=50,c=50,u;for(let p of t){if(a.includes(p))u=s.find(l=>l.id===p);else{let l=e.find(d=>d.source===p);u={id:p,x:+(l==null?void 0:l.x)||i+10*Math.random(),y:+(l==null?void 0:l.y)||c+10*Math.random()},(l!=null&&l.x||l!=null&&l.y)&&(u.fx=+(l==null?void 0:l.x),u.fy=+(l==null?void 0:l.y))}r.push(u)}this.store.dispatch("setStagedChanges",{staged_changes:{edges:e,nodes:r}})}}),Bt=new U({id:"land-surface-file-loader",title:"Load Land Surfaces (.csv)",primary_callback:V,data_callback:function(e){var u,p;let t=[...new Set(e.map(l=>l.node_id))],s=w(m(this,"store.state.graph.nodes")),a=w(m(this,"store.state.graph.edges")).map(l=>({source:l.source.id,target:l.target.id})),r=s.map(l=>l.id),i=[],c=()=>400*Math.random();for(let l of t){let d=e.filter(y=>y.node_id===l),o,f=H((u=d.filter(y=>y==null?void 0:y.long))==null?void 0:u.map(y=>+y.long)),g=H((p=d.filter(y=>y==null?void 0:y.lat))==null?void 0:p.map(y=>+y.lat)),S=d.map(({node_id:y,surface_key:k,area_acres:O,imp_area_acres:b})=>({node_id:y,surface_key:k,area_acres:O,imp_area_acres:b}));r.includes(l)?(o=s.find(y=>y.id===l),o.node_type="land_surface",o.data=S,f&&g&&(o.longlat=[+f,+g])):(o={id:l,node_type:"land_surface",x:c(),y:c(),data:S},f&&g&&(o.longlat=[+f,+g]),i.push(o))}s=s.concat(i),this.store.dispatch("setStagedChanges",{staged_changes:{edges:a,nodes:s}})}}),Ut=new U({id:"treatment-facilities-file-loader",title:"Load Treatment Facilities (.csv)",primary_callback:V,data_callback:function(e){let t=[...new Set(e.map(u=>u.node_id))],s=w(m(this,"store.state.graph.nodes")||[{}]),a=w(m(this,"store.state.graph.edges")||[{}]).map(u=>({source:u.source.id,target:u.target.id})),r=s.map(u=>u.id),i=[],c=()=>400*Math.random();for(let u of t){let p=e.find(f=>f.node_id===u),l,{long:d,lat:o}=p;r.includes(u)?(l=s.find(f=>f.id===u),l.data=p,l.node_type="treatment_facility",d&&o&&(l.longlat=[+d,+o])):(l={id:u,node_type:"treatment_facility",x:c(),y:c(),data:p},d&&o&&(l.longlat=[+d,+o]),i.push(l))}s=s.concat(i),this.store.dispatch("setStagedChanges",{staged_changes:{edges:a,nodes:s}})}}),Wt=new U({id:"scenario-file-loader",title:"Load Scenario (.json)",primary_callback:V,secondary_callback:ue,data_callback:function(e){let t=e.graph,s=e.name,a=w(t.nodes||[{}]);for(let i of a)i.id==null&&console.error("bad egg:",i);let r=t.edges.map(i=>({source:i.source.id,target:i.target.id}));this.store.dispatch("setStagedChanges",{staged_changes:{edges:r,nodes:a,scenario_name:s}})}}),qt=new wt({id:"load-table-drawer",title:"Load from File",children:[Vt,Bt,Ut,Wt]});class Ht extends v{constructor(e){super({store:_,id:e.id})}get scenario_name(){return m(this.store,"state.scenario_name")}get scenario(){return{name:this.scenario_name,graph:m(this,"store.state.graph")}}saveScenarioBlob(){let e={filename:(this.scenario.name.replaceAll(" ","_")||"scenario")+".json",json:JSON.stringify(this.scenario,void 0,2)},t=new Blob([e.json],{type:"text/plain;charset=utf-8"});W.saveAs(t,e.filename)}dumpScenario(){let e=`<pre>${JSON.stringify(this.scenario,void 0,2)}</pre>`;this.element.select("#dump-scenario-text").html(e)}clearScenario(){this.element.select("#dump-scenario-text").html("")}_template(){return`
<div>
  ...
  `}_render(){let e=this;e.element=h.select(`#${e.parent_id}-content`).append("div").attr("id",e.id),e.element.html(e._template()),e.button=e.element.select("#save-scenario"),e.button.on("click",e.saveScenarioBlob.bind(e)),e.button=e.element.select("#dump-scenario"),e.button.on("click",e.dumpScenario.bind(e)),e.button=e.element.select("#clear-scenario"),e.button.on("click",e.clearScenario.bind(e))}}class Gt extends v{constructor(e){super({store:_,id:e.id})}get facility_types(){return Object.keys(m(this,"store.state.facility_type_map"))}get facility_type_map(){return m(this,"store.state.facility_type_map")}get schema(){return m(this,"store.state.schema")}get scenario_name(){return m(this.store,"state.scenario_name")}get facility_properties(){let e=new Set;for(let t of this.facility_types){let s=this.facility_type_map[t],a=this.schema[s];Object.keys(a.properties).forEach(e.add,e)}return["long","lat"].forEach(e.add,e),e}get facility_data(){let e=this.facility_properties,t=[],s=w(m(this,"store.state.graph.nodes")||[{}]).filter(a=>a.node_type=="treatment_facility");return s.length==0&&(s=[{}]),s.forEach(a=>{let r={};for(let c of e)r[c]=m(a.data,c)||"";let i=(a==null?void 0:a.longlat)||["",""];r.long=i[0],r.lat=i[1],t.push(r)}),t}facility_template(){let e=this,t=[],s=m(e,"store.state.nereid_state"),a=m(e,"store.state.nereid_region"),r=m(e,`store.state.treatment_facility_fields.${s}.${a}`)||m(e,"store.state.treatment_facility_fields.state.region"),i=(r==null?void 0:r.ignored)||[],c=(r==null?void 0:r.disabled)||[],u=[...this.facility_properties].filter(p=>!i.includes(p));for(let[p,l]of Object.entries(this.facility_type_map)){let d=this.schema[l],o={};for(let f of u){let g=m(d.properties[f],"type")||"";d.required.includes(f)&&(g+="-req"),c.includes(f)&&(g+="-uneditable"),o[f]=g}o.long="number",o.lat="number",o.facility_type=p,t.push(o)}return t}saveTreatmentCSV(){let e=this.facility_data,s={filename:`${this.scenario_name.replaceAll(" ","-")}-treatment_facilities.csv`,csv:G({data:e})},a=new Blob([s.csv],{type:"text/plain;charset=utf-8"});W.saveAs(a,s.filename)}saveTreatmentTemplateCSV(){let e=this.facility_template(),t=m(this,"store.state.nereid_state"),s=m(this,"store.state.nereid_region"),a={filename:`${t}-${s}-treatment_facilities-template.csv`,csv:G({data:e})},r=new Blob([a.csv],{type:"text/plain;charset=utf-8"});W.saveAs(r,a.filename)}_template(){return`
<div>
...
<div>
  <div class="flex flex-row w-full p-4 justify-between items-center"><strong>Save Treatment Facility Template to File (csv)</strong>
    ...
    `}_render(){let e=this;e.element=h.select(`#${e.parent_id}-content`).append("div").attr("id",e.id),e.element.html(e._template()),e.store.events.subscribe("updateConfig",()=>{}),e.element.select("#save-tmnt").on("click",e.saveTreatmentCSV.bind(e)),e.element.select("#save-tmnt-template").on("click",e.saveTreatmentTemplateCSV.bind(e))}}const Jt=new Ht({id:"save-scenario"}),Xt=new Gt({id:"save-tmnt-facility"}),Qt=new xt({id:"save-files-drawer",title:"Save to File",children:[Jt,Xt]}),Yt=new kt({children:[$t,It,qt,Qt]}),Zt=new He({id:"editor-tab",children:[Ke,R,ot,dt,yt,Yt]});async function Q(n,e){let{default:t}=await J(async()=>{const{default:l}=await import("./xlsx-CYBbodID.js");return{default:l}},[]),{Tabulator:s,DownloadModule:a,ExportModule:r,FormatModule:i,ResizeColumnsModule:c}=await J(async()=>{const{Tabulator:l,DownloadModule:d,ExportModule:o,FormatModule:f,ResizeColumnsModule:g}=await import("./tabulator-K2q2Ulc8.js");return{Tabulator:l,DownloadModule:d,ExportModule:o,FormatModule:f,ResizeColumnsModule:g}},[]);s.registerModule([a,r,i,c]);let p=Object.assign({data:[{}],maxHeight:"500px",minHeight:80,autoResize:!1,layout:"fitData",responsiveLayout:!1,history:!1,tooltipsHeader:!0,footerElement:'<div class="tabulator-footer"></div>',columns:[],initialSort:[{column:"node_id",dir:"asc"}],dependencies:{XLSX:t}},e);return new s(n,p)}class fe extends v{constructor(e){super({store:_,id:e.id});let t=this;t.table_builders=e.table_builders||[],t.tables=[]}get scenario_name(){return m(this.store,"state.scenario_name")}async buildResultsSummary({id:e,data:t,prep_fnx:s,title:a,description:r,filename_csv:i}){let c=this;c.element.select(`#${e}`).remove();let u=c.element.append("div").attr("id",e).classed("grid grid-cols-1 grid-rows-auto pt-8",!0),p=u.append("div").classed(" flex flex-row w-full justify-between items-center",!0);p.append("div").classed("font-bold",!0).html(a);const[l,d]=s(t);if(!l.length){u.append("div").classed("flex justify-center",!0).html("no results to show for this summary yet...");return}let o=p.append("div").classed("py-2",!0).append("button").text("Download Data (csv)").classed("btn btn-blue",!0);r!=null&&u.append("div").html(r);const f=e+"-results-tabulator";u.append("div").attr("id",f);let g=await Q(`#${f}`,{data:l,columns:d.map(y=>{let k={title:y.replaceAll("_"," "),field:y,titleDownload:y};return["_acres","_coeff","_load","_pct","_conc","_lbs","_cuft","_cfs","_mpn","_inhr"].some(b=>y.includes(b))&&(k.formatter="money",k.formatterParams={precision:2}),y.includes("_pct")&&(k.formatterParams={precision:2,symbol:"%",symbolAfter:"p"}),y.includes("_mpn")&&(k.formatterParams={precision:0}),k})});c.tables.push(g);let S=c.scenario_name.replaceAll(" ","-");o.on("click",()=>{g.download("csv",S+"-"+i)})}async update(e){let t=this;if(e=e||[],t.element.html(""),(e==null?void 0:e.length)>0){t.element.select("#dummy_table").remove(),t.element.append("div").attr("id","dummy_table").classed("hidden",!0);let s=await Q("#dummy_table");t.element.append("div").append("button").classed("btn btn-blue",!0).text("Download All Data Summaries (xlsx)").on("click",()=>{let r={};t.element.selectAll("[id$='-results-tabulator']").nodes().forEach(i=>{let c=h.select(i).attr("id"),u=c.replace("-results-tabulator","").replace("table-","").replace("facility-","bmp-").slice(0,30);c.includes("table-all-data")||(r[u]="#"+c)}),t.element.select("#dummy_table").remove(),t.element.append("div").attr("id","dummy_table").classed("hidden",!0),s.download("xlsx","AllData.xlsx",{sheets:r})})}F();try{for(let s of t.table_builders)await t.buildResultsSummary(s(e))}finally{P()}}_template(){return`
      <div>Click 'Run Scenario' in the Editor to calculate results.</div>
    `}_render(){let e=this;e.element=h.select(`#${e.id}`).classed("relative flex justify-center",!0).append("div").classed("flex flex-col relative max-w-screen-md py-10 ",!0).html(e._template()),e.store.events.subscribe("newResults",({results:t})=>e.update(t)),e.store.events.subscribe("changedTab",({current_tab:t})=>{if(t===e.id){let s=m(e,"store.state.results");e.update(s)}})}}const Kt=n=>({data:n,id:"table-facility-design-summary",title:"Facility design Summary",filename_csv:"facility_design_summary.csv",prep_fnx:t=>{const s=["node_id","facility_type","valid_model","design_intensity_inhr","design_volume_cuft_cumul"];return[(t||[]).filter(r=>r.node_type).filter(r=>r.node_type.includes("facility")).map(r=>Object.fromEntries(Object.entries(r).filter(([i,c])=>s.includes(i)))),s]}}),es=n=>({data:n,id:"table-facility-wet-weather-capture",title:"Facility Wet Weather Volume Capture Results",filename_csv:"wet_weather_volume_capture_results.csv",prep_fnx:t=>{const s=["node_id","facility_type","valid_model","captured_pct","treated_pct","retained_pct","bypassed_pct","peak_flow_mitigated_pct"];return[(t||[]).filter(r=>r.node_type).filter(r=>r.node_type.includes("facility")).map(r=>Object.fromEntries(Object.entries(r).filter(([i,c])=>s.includes(i)))),s]}});function ts(n){return{data:n,id:"table-facility-volume-reduction",title:"Facility Volume Reduction Results",filename_csv:"volume_reduction_results.csv",prep_fnx:t=>{let s=["node_id"];const a=["inflow","treated","retained","captured","bypassed"],i=[...new Set(x(t.map(d=>Object.keys(d))))].filter(d=>{let o=a.some(g=>d.endsWith(`dry_weather_flow_cuft_${g}`)),f=a.some(g=>d.endsWith(`runoff_volume_cuft_${g}`));return o||f}),c=function(d){return s.includes(d)||i.includes(d)};let u=(t||[]).filter(d=>d.node_type).filter(d=>d.node_type.includes("facility")).map(d=>Object.fromEntries(Object.entries(d).filter(([o,f])=>c(o))));const p=[...new Set(x(u.map(d=>Object.keys(d))))],l=s;for(let d of a){let o=p.filter(f=>f.includes(d)).sort((f,g)=>f.includes("winter")||f.includes("summer")?1:-1);l.push(...o)}return[u,l]}}}function ss(n){return{data:n,id:"table-facility-wet-weather-volume-reduction",title:"Facility Wet Weather Volume Reduction Results",filename_csv:"wet_weather_volume_reduction_results.csv",prep_fnx:t=>{let s=["node_id"];const a=["inflow","treated","retained","captured","bypassed"],i=[...new Set(x(t.map(d=>Object.keys(d))))].filter(d=>(a.some(f=>d.endsWith(`dry_weather_flow_cuft_${f}`)),a.some(f=>d.endsWith(`runoff_volume_cuft_${f}`)))),c=function(d){return s.includes(d)||i.includes(d)};let u=(t||[]).filter(d=>d.node_type).filter(d=>d.node_type.includes("facility")).map(d=>Object.fromEntries(Object.entries(d).filter(([o,f])=>c(o))));const p=[...new Set(x(u.map(d=>Object.keys(d))))],l=s;for(let d of a){let o=p.filter(f=>f.includes(d));l.push(...o)}return[u,l]}}}function as(n){return{data:n,id:"table-facility-dry-weather-volume-reduction",title:"Facility Dry Weather Volume Reduction Results",description:"(calculated as total of summer + winter)",filename_csv:"dry_weather_volume_reduction_results.csv",prep_fnx:t=>{let s=["node_id"];const a=["inflow","treated","retained","captured","bypassed"],i=[...new Set(x(t.map(d=>Object.keys(d))))].filter(d=>{let o=a.some(f=>d.endsWith(`dry_weather_flow_cuft_${f}`));return a.some(f=>d.endsWith(`runoff_volume_cuft_${f}`)),o}),c=function(d){return s.includes(d)||i.includes(d)};let u=(t||[]).filter(d=>d.node_type).filter(d=>d.node_type.includes("facility")).map(d=>Object.fromEntries(Object.entries(d).filter(([o,f])=>c(o))));for(let d of u)for(let o of a){let f=d[`summer_dry_weather_flow_cuft_${o}`],g=d[`winter_dry_weather_flow_cuft_${o}`];d[`total_dry_weather_volume_cuft_${o}`]=f+g}const p=[...new Set(x(u.map(d=>Object.keys(d))))],l=s;for(let d of a){let o=p.filter(f=>f.includes(d)).sort((f,g)=>f.includes("total")||f.includes("winter")||f.includes("summer")?1:-1);l.push(...o)}return[u,l]}}}function rs(n){return{data:n,id:"table-facility-total-volume-reduction",title:"Facility Total Volume Reduction Results",description:"(calculated as total of results for wet + summer + winter)",filename_csv:"total_volume_reduction_results.csv",prep_fnx:t=>{let s=["node_id"];const a=["inflow","treated","retained","captured","bypassed"],i=[...new Set(x(t.map(d=>Object.keys(d))))].filter(d=>{let o=a.some(g=>d.endsWith(`dry_weather_flow_cuft_${g}`)),f=a.some(g=>d.endsWith(`runoff_volume_cuft_${g}`));return o||f}),c=function(d){return s.includes(d)||i.includes(d)};let u=(t||[]).filter(d=>d.node_type).filter(d=>d.node_type.includes("facility")).map(d=>Object.fromEntries(Object.entries(d).filter(([o,f])=>c(o))));for(let d of u)for(let o of a){let f=d[`runoff_volume_cuft_${o}`],g=d[`summer_dry_weather_flow_cuft_${o}`],S=d[`winter_dry_weather_flow_cuft_${o}`];d[`total_volume_cuft_${o}`]=f+g+S}const p=[...new Set(x(u.map(d=>Object.keys(d))))],l=s;for(let d of a){let o=p.filter(f=>f.includes(d)).filter(f=>f.includes("total"));l.push(...o)}return[u,l]}}}function ns(n){return{data:n,id:"table-facility-load-reduction",title:"Facility Load Reduction Results",description:`These results are separated in to wet weather results,
    summer dry weather results, and winter dry weather results.`,filename_csv:"load_reduction_results.csv",prep_fnx:t=>{const s=["node_id"],a=function(l){let d=l.endsWith("lbs_removed")||l.endsWith("mpn_removed");return s.includes(l)||d},i=[...new Set(x(t.map(l=>Object.keys(l))))].filter(l=>{let d=l.endsWith("lbs_removed")||l.endsWith("mpn_removed"),o=!l.includes("dw");return d&&o});let c=(t||[]).filter(l=>l.node_type).filter(l=>l.node_type.includes("facility")).map(l=>Object.fromEntries(Object.entries(l).filter(([d,o])=>a(d))));const u=[...new Set(x(c.map(l=>Object.keys(l))))],p=s;for(let l of i.sort()){let d=u.filter(o=>o.includes(l)).sort((o,f)=>o.includes("total")||o.includes("winter")||o.includes("summer")?1:-1);p.push(...d)}return[c,p]}}}function is(n){return{data:n,id:"table-facility-total-load-reduction",title:"Facility Total Load Reduction Results",description:"(calculated as total of results for wet + summer + dry)",filename_csv:"total_load_reduction_results.csv",prep_fnx:t=>{let s=["node_id"];const r=[...new Set(x(t.map(l=>Object.keys(l))))].filter(l=>{let d=l.endsWith("lbs_removed")||l.endsWith("mpn_removed"),o=!l.includes("dw");return d&&o}),i=function(l){let d=l.endsWith("lbs_removed")||l.endsWith("mpn_removed");return s.includes(l)||d};let c=(t||[]).filter(l=>l.node_type).filter(l=>l.node_type.includes("facility")).map(l=>Object.fromEntries(Object.entries(l).filter(([d,o])=>i(d))));for(let l of c)for(let d of r){let o=l[d],f=l[`summer_dw${d}`],g=l[`winter_dw${d}`];l[`total_${d}`]=o+f+g}const u=[...new Set(x(c.map(l=>Object.keys(l))))],p=s;for(let l of r.sort()){let d=u.filter(o=>o.includes(l)).filter(o=>o.includes("total")).sort((o,f)=>o.includes("total")||o.includes("winter")||o.includes("summer")?1:-1);p.push(...d)}return[c,p]}}}function os(n){return{data:n,id:"table-facility-wet-weather-load-reduction",title:"Facility Wet Weather Load Reduction Results",filename_csv:"wet_weather_load_reduction_results.csv",prep_fnx:t=>{const s=function(i){let c=["node_id"].includes(i),u=i.endsWith("lbs_removed")||i.endsWith("mpn_removed"),p=i.includes("dw");return c||u&&!p};let a=(t||[]).filter(i=>i.node_type).filter(i=>i.node_type.includes("facility")).map(i=>Object.fromEntries(Object.entries(i).filter(([c,u])=>s(c))));const r=[...new Set(x(a.map(i=>Object.keys(i))))];return[a,r]}}}function ls(n){return{data:n,id:"table-facility-dry-weather-load-reduction",title:"Facility Dry Weather Load Reduction Results",description:`Includes summer dry weather, winter dry weather, and total dry
      weather results (calculated as summer + winter)`,filename_csv:"dry_weather_load_reduction_results.csv",prep_fnx:t=>{let s=["node_id"];const r=[...new Set(x(t.map(l=>Object.keys(l))))].filter(l=>{let d=l.endsWith("lbs_removed")||l.endsWith("mpn_removed"),o=!l.includes("dw");return d&&o}),i=function(l){let d=l.endsWith("lbs_removed")||l.endsWith("mpn_removed"),o=l.includes("dw");return s.includes(l)||d&&o};let c=(t||[]).filter(l=>l.node_type).filter(l=>l.node_type.includes("facility")).map(l=>Object.fromEntries(Object.entries(l).filter(([d,o])=>i(d))));for(let l of c)for(let d of r){let o=l[`summer_dw${d}`],f=l[`winter_dw${d}`];l[`total_dw${d}`]=o+f}const u=[...new Set(x(c.map(l=>Object.keys(l))))],p=s;for(let l of r.sort()){let d=u.filter(o=>o.includes(l)).sort((o,f)=>o.includes("total")||o.includes("winter")||o.includes("summer")?1:-1);p.push(...d)}return[c,p]}}}const ds=n=>({data:n,id:"table-land-surface-summary",title:"Land Surface Summary",filename_csv:"land_surface_summary.csv",prep_fnx:t=>{let s=["node_id","area_acres","ro_coeff","imp_pct"];return[(t||[]).filter(r=>(r==null?void 0:r.land_surfaces_count)>0).map(r=>Object.fromEntries(Object.entries(r))),s]}}),cs=n=>({data:n,id:"table-land-surface-load-summary",title:"Land Surface Loading Summary",filename_csv:"land_surface_load_summary.csv",prep_fnx:t=>{let s=["node_id"];const a=function(c){let u=c.endsWith("_load_lbs")||c.endsWith("_load_mpn");return s.includes(c)||u};let r=(t||[]).filter(c=>(c==null?void 0:c.land_surfaces_count)>0).map(c=>Object.fromEntries(Object.entries(c).filter(([u,p])=>a(u))));const i=[...new Set(x(r.map(c=>Object.keys(c))))];return console.debug("landsurface load summary:",r),[r,i]}}),us=[Kt,es,ts,rs,ss,as,ns,is,os,ls],ps=[ds,cs],ms=new fe({id:"treatment-facility-results-tab",table_builders:us}),fs=new fe({id:"land-surface-results-tab",table_builders:ps});class _s extends v{constructor(e){super({store:_,id:e.id})}_template(){return`
    <div class="markdown">
